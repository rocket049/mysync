...
type FilterWalker struct {
	ResMap   map[string]files.FileDesc
	OldMap   map[string]files.FileDesc
	StartPos int
}

func NewFilterWalker(path1 string) (res *FilterWalker) {
	res = new(FilterWalker)
	res.ResMap = make(map[string]files.FileDesc)
	var err error
	res.OldMap, err = GetCachedFileMap(path1)
	if err != nil {
		res.OldMap = make(map[string]files.FileDesc)
	}
	if path1 == "." {
		res.StartPos = 0
	} else {
...
func (p *FilterWalker) Walker(path1 string, info os.FileInfo, err error) error {
	var fd1 files.FileDesc
	upath := files.GetUnixPath(path1[p.StartPos:])
	...
	if ok == false {
		fd1.MD5 = files.GetFileMD5(path1)
		p.ResMap[upath] = fd1
...
func UpdateFileMap(rootpath string) (res map[string]files.FileDesc, err error) {
